  ...
  module StorageTypes
    class PostgresStorage < Storage
      ...
      def select_data(type, id, pkeys, xmin, xmax, boundary)
        column_classes = self.column_classes(type)
        select_query = select_data_queries[type][boundary] or raise("invalid boundary condition: #{boundary.inspect}")
        rows = execute(select_query, id, xmin, xmax)
        rows.map do |row|
          row.values.each_with_index.map do |value, i|
            column_classes[i].deserialize(value)
          end
        end
      end

      def insert_datum(type, id, pkey, *datum)
        execute(insert_datum_queries[type], id, *datum)
      end

      def insert_datum_async(type, id, pkey, *datum)
        execute_async(insert_datum_queries[type], id, *datum)
      end
      ...
      def select_data_queries
        @select_data_queries ||= Hash.new do |cache, type|
          ...
          cache[type] = {
            "[]" => "select #{column_names.join(', ')} from #{table_name} where id = $1 and x >= $2 and x <= $3",
            "[)" => "select #{column_names.join(', ')} from #{table_name} where id = $1 and x >= $2 and x <  $3",
            "(]" => "select #{column_names.join(', ')} from #{table_name} where id = $1 and x >  $2 and x <= $3",
            "()" => "select #{column_names.join(', ')} from #{table_name} where id = $1 and x >  $2 and x <  $3",
          }
      ...
      def insert_datum_queries
        @insert_datum_queries ||= Hash.new do |cache, type|
          table_name   = self.class.table_name_for(type)
          column_names = self.class.column_names_for(type)
          cache[type]  = "insert into #{table_name} (id, #{column_names.join(', ')}) values ($1, #{column_names.each_with_index.map {|s, i| "$#{i + 2}"}.join(', ')})"
        end
      end
    end
  end
