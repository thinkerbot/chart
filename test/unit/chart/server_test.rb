...
class Chart::ServerTest < Minitest::Test
  ...
  def driver
    @driver ||= Chart::Driver.new(storage)
  end
  ...
  def test_index_lists_topics_by_id
    driver.create(topic_type, test_id("a"))
    driver.create(topic_type, test_id("b"))

    get '/topics'
    assert last_response.body.include?(test_id("a"))
    assert last_response.body.include?(test_id("b"))
  end
  ...
  def test_get_existing_topic_returns_ok
    driver.create(topic_type, test_id)
    get "/topics/#{test_id}"
    assert last_response.ok?
  end
  ...
  def test_post_to_topic_id_creates_topic
    assert_equal nil, driver.find(test_id)
    post "/topics/#{test_id}"
    assert_equal test_id, driver.find(test_id).id
  end

  def test_post_to_topic_id_with_config_creates_topic_with_config
    post "/topics/#{test_id}", :topic => {:config => {"a" => "A"}}
    assert_equal "A", driver.find(test_id).config["a"]
  end

  def test_post_to_existing_topic_id_causes_error_and_does_nothing
    post "/topics/#{test_id}", :topic => {:config => {"a" => "A"}}
    post "/topics/#{test_id}", :topic => {:config => {"a" => "B"}}
    assert !last_response.ok?
    assert_equal "A", driver.find(test_id).config["a"]
  end
  ...
  def test_post_guesses_ii_type_if_unspecified
    post "/topics/#{test_id}"
    assert_equal "ii", driver.find(test_id).type
  end
  ...
  def create_ii_topic
    topic = driver.create(topic_type, test_id)
    topic.save_data([
      [0, 1],
      [1, 2],
      [2, 3],
  ...
  def test_get_data_returns_data_in_min_max_range_in_reverse_xz_order
    ...
    get "/data/#{test_id}?x=[1,2]"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=[1,3)"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=(0,2]"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=(0,3)"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)
  end
  ...
  def test_get_data_returns_data_in_min_offset_range_in_reverse_xz_order
    ...
    # minus
    get "/data/#{test_id}?x=[2~-1]"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=[3~-2)"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=(2~-2]"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)

    get "/data/#{test_id}?x=(3~-3)"
    assert_equal reverse_data_from_1_to_2, JSON.parse(last_response.body)
  end
  ...
  def test_get_data_with_a_projection
    ...
    get "/data/#{test_id}?projection=histogram"
    assert_equal [
      [4,1],
      [3,1],
      [2,1],
